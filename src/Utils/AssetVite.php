...
namespace Yabe\Movebender\Utils;
...
class AssetVite
{
    ...
    {
        return $this->_enqueue_asset(dirname(MOVEBENDER::FILE) . '/build', $entry, $options);
    }
    ...
    {
        return $this->_register_asset(dirname(MOVEBENDER::FILE) . '/build', $entry, $options);
    }

    public function get_assets(string $entry, array $options)
    {
        return $this->_get_assets(dirname(MOVEBENDER::FILE) . '/build', $entry, $options);
    }
    ...
    {
        return plugins_url('build/', MOVEBENDER::FILE);
    }
    ...
    public function get_vite_client_script(object $manifest): string
    {
        // if (wp_script_is(self::VITE_CLIENT_SCRIPT_HANDLE)) {
        //     return;
        // }
    ...
    public function get_development_asset(object $manifest, string $entry, array $options): ?array
    {
        $vite_client_script = $this->get_vite_client_script($manifest);
        // $this->get_react_refresh_preamble_script($manifest);
        ...
        $assets = [
            'scripts' => [$vite_client_script, $src],
            'styles' => $options['css-dependencies'],
        ];

        return $assets;
        // return [
        //     'vite_client_script' => $vite_client_script,
        //     'assets' => $assets,
        //     'src' => $src,
        // ];
    }
    ...
    public function get_production_asset(object $manifest, string $entry, array $options)
    {
        $url = $this->prepare_asset_url($manifest->dir);
        ...
        if (!empty($item->css)) {
            foreach ($item->css as $index => $css_file_path) {
                ...
                //     $assets['styles'][] = $style_handle;
                // }
                $assets['styles'][] = "{$url}/{$css_file_path}";
            }
        }
     ...
     * @return array
     */
    public function _get_assets(string $manifest_dir, string $entry, array $options)
    {
        try {
            $manifest = $this->get_manifest($manifest_dir);
        } catch (Exception $e) {
        ...
        return $manifest->is_dev
            ? $this->get_development_asset($manifest, $entry, $options)
            : $this->get_production_asset($manifest, $entry, $options);
    }
