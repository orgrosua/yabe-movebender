...
class Common
{
    ...
    public static function plugin_data(?string $key = null)
    {
        ...
        $plugin_data = wp_cache_get('plugin_data', MOVEBENDER::WP_OPTION);

        if (! $plugin_data) {
            $plugin_data = get_plugin_data(MOVEBENDER::FILE);
            wp_cache_set('plugin_data', $plugin_data, MOVEBENDER::WP_OPTION);
        }
