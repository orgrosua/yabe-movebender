...
class Requirement
{
    ...
    public function plugins(array $plugins): self
    {
        ...
        foreach ($plugins as $plugin) {
            if (! array_key_exists($plugin['slug'], $installed_plugins) && ! in_array($plugin['slug'], $installed_plugins, true)) {
                $this->doesNotMeet[] = sprintf('<strong>%s</strong> plugin are not installed', esc_html($plugin['name']));
                continue;
            }
    ...
    {
        // $name = esc_html(get_plugin_data(MOVEBENDER::FILE, false)['Name']);
        ...
        $notice = sprintf(
            ...
            esc_html(get_plugin_data(MOVEBENDER::FILE, false)['Name'])
        );
